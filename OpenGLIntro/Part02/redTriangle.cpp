...
int main( void )
{
    ...
    ColorShader* clShader = new ColorShader(glm::vec4(0.3,0.4,0.9,1.0));

    myTriangle->setShader(shader);
    secondTriangle->setShader(clShader);
    
    myTriangle->setTranslate(glm::vec3(-1.6,0,0)); // << shift to left
    secondTriangle->setTranslate(glm::vec3(1.6,0,0));

    /* QUAD */
    Quad* quad = new Quad();
    ColorShader* clShader2 = new ColorShader(glm::vec4(1,0,0,1));
    quad->setShader(clShader2);
    quad->setTranslate(glm::vec3(0,0,1));
