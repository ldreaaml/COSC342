...
// Uncomment for Microsoft Visual Studio to enable secured fopen and sprint
...
Pixel& Image::operator()(const Vector &v) {
	int x = int (v(0) + 0.5);
	int y = int (v(1) + 0.5);
...
const Pixel& Image::operator()(const Vector &v) const {
	int x = int(v(0) + 0.5);
	int y = int(v(1) + 0.5);
